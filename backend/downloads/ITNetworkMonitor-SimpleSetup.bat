@echo off
echo ========================================
echo IT Network Monitor Agent - Simple Setup
echo ========================================
echo.
...
REM Download the agent from the server
echo Downloading agent from server...
set BACKEND_URL=https://itmanagement.bylinelms.com
if "%1"=="dev" set BACKEND_URL=http://localhost:5001

echo Downloading network_monitor_agent.py...
echo URL: %BACKEND_URL%/downloads/network_monitor_agent.py
curl -s -o "%INSTALL_DIR%\network_monitor_agent.py" "%BACKEND_URL%/downloads/network_monitor_agent.py"
if %errorLevel% neq 0 (
...
REM Check if the downloaded file is valid Python
echo Checking downloaded file...
findstr /C:"#!/usr/bin/env python3" "%INSTALL_DIR%\network_monitor_agent.py" >nul
if %errorLevel% neq 0 (
    echo ✗ Downloaded file is not a valid Python agent
    echo The file may be an error page instead of the agent
    echo Please check the server configuration
    echo.
    echo First few lines of downloaded file:
    type "%INSTALL_DIR%\network_monitor_agent.py" | more +0
    pause
    exit /b 1
)

echo ✓ Agent file downloaded and verified successfully
...
echo python network_monitor_agent.py
