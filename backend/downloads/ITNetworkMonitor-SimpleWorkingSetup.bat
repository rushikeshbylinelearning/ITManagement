@echo off
echo ========================================
echo IT Network Monitor Agent - Simple Working Setup
echo ========================================
echo.
...
curl -o "%INSTALL_DIR%\network_monitor_agent.py" "http://localhost:5001/downloads/network_monitor_agent.py" >nul 2>&1
if %errorLevel% == 0 (
    ...
    powershell -Command "Invoke-WebRequest -Uri 'http://localhost:5001/downloads/network_monitor_agent.py' -OutFile '%INSTALL_DIR%\network_monitor_agent.py'" >nul 2>&1
    if %errorLevel% == 0 (
        echo ✓ Agent file downloaded via PowerShell
    ) else (
        echo ✗ Failed to download agent file
...
echo ✓ Downloaded agent file from server
