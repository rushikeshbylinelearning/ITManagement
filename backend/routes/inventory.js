...
// ✅ SECURED: All inventory management routes are now protected and restricted
// to admins and technicians. This single line replaces the old middleware.
...
router.put('/bulk-update', async (req, res) => {
    ...
    try {
        ...
        const result = await InventoryItem.updateMany( { _id: { $in: ids } }, updatePayload );
...
router.post('/bulk-delete', async (req, res) => {
    ...
    try {
        const assignedItems = await InventoryItem.find({ _id: { $in: ids }, status: 'Assigned' });
...
// GET ONLY UNASSIGNED inventory items
router.get('/unassigned', async (req, res) => {
    try {
        const unassignedItems = await InventoryItem.find({ status: 'Unassigned' }).sort({ componentType: 1, brand: 1 });
...
router.get('/', async (req, res) => {
    try {
        const items = await InventoryItem.find().populate('assignedTo', 'name email').sort({ componentType: 1, createdAt: -1 });
        res.json(items);
    } catch (err) {
...
router.get('/:id', async (req, res) => {
    ...
    try {
        const item = await InventoryItem.findById(req.params.id).populate('assignedTo', 'name email');
...
router.post('/', async (req, res) => {
    try {
        ...
        if (serialNumber) {
            ...
            if (snExists) return res.status(409).json({ msg: `Serial number '${serialNumber}' already exists.` });
        }
...
router.put('/:id', async (req, res) => {
    ...
    try {
        ...
        if (componentType) {
            const typeExists = await ComponentType.findOne({ name: componentType });
            if (!typeExists) return res.status(400).json({ msg: `Component type '${componentType}' is not recognized.` });
        }
        if (serialNumber) {
            const snExists = await InventoryItem.findOne({ serialNumber, _id: { $ne: req.params.id } });
            if (snExists) return res.status(409).json({ msg: `Serial number '${serialNumber}' is already in use by another item.` });
        }
